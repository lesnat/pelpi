...
class test_PelpiObject(unittest.TestCase):
    ...
    def test_initialize_defaults(self):
        ...
        self.assertEqual(self.po.default.get(key='all'),{})
        
        self.po._initialize_defaults(input_dict={"test_input":1.0})
        self.assertEqual(self.po.default.get(key='all'),{'test_input':1.0})
        
        self.po.test_attribute = 0.0
        self.po.test_method = self.test_check_input # instance method
        self.po._initialize_defaults(input_dict={"test_input":1.0})
        self.assertEqual(self.po.default.get(key='all'),{'test_input':1.0,'test_method':None})
